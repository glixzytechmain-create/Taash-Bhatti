...
export default function NaanEscape404({ onBackToFeast }: NaanEscape404Props) {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      const glowGrad = ctx.createRadialGradient(width * 0.75, groundY - 40, 20, width * 0.75, groundY - 40, 260);
      ...
      for (let i = 0; i < 5; i++) {
        const xPos = ((i * 200 - (s.backgroundOffset % 200) + width) % width);
        // Chimney flue pipe
        ctx.fillRect(xPos + 40, groundY - 95, 22, 95);
        // Copper Handi dome silhouette
        ctx.beginPath();
        ctx.arc(xPos + 51, groundY - 100, 16, Math.PI, 0);
        ctx.fill();
      }
      ...
      for (let i = 0; i < 18; i++) {
        const gx = ((i * 50 - (s.groundOffset % 50) + width) % width);
        ctx.fillRect(gx, groundY + 4 + (i % 3) * 6, (i % 2 === 0 ? 14 : 8), 2.5);
      }
      ...
      if (s.gameState === 'playing') {
        ...
        if (s.lastSpawnDistance > s.nextSpawnGap) {
          s.lastSpawnDistance = 0;
          s.nextSpawnGap = Math.random() * 160 + 210; // fair gap
      ...
      if (p.isGrounded && s.gameState === 'playing') {
        const footOffset1 = Math.sin(p.runCycle) * 7;
        const footOffset2 = -Math.sin(p.runCycle) * 7;

        ctx.fillStyle = '#ffffff';
        // Left Foot
        ctx.beginPath();
        ctx.ellipse(-14, p.height / 2 - 2 + footOffset1, 6, 4, 0, 0, Math.PI * 2);
        ctx.fill();
        ...
        // Right Foot
        ctx.beginPath();
        ctx.ellipse(14, p.height / 2 - 2 + footOffset2, 6, 4, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.stroke();
      }
      ...
      naanGrad.addColorStop(0, '#fff4db'); // Soft puffed flour center
      naanGrad.addColorStop(0.55, '#f5be6c'); // Golden baked wheat
      naanGrad.addColorStop(0.88, '#d68336'); // Crispy tandoor crust edge
      naanGrad.addColorStop(1, '#9b4a1b'); // Dark scorched edge
      ...
      const blisters = [
        { x: -12, y: -10, r: 4.5, cr: '#210d04' },
        { x: 12, y: -8, r: 5.5, cr: '#2a1105' },
        { x: -14, y: 8, r: 6.0, cr: '#381608' },
        { x: 15, y: 7, r: 4.0, cr: '#2a1105' },
        { x: 2, y: 12, r: 5.0, cr: '#210d04' },
        { x: -2, y: -13, r: 3.5, cr: '#421d0a' },
      ];
      blisters.forEach((b) => {
        ctx.beginPath();
        ...
        ctx.fillStyle = b.cr;
        ctx.fill();
        // Golden roasted halo around blister
      ...
      ctx.fill();
      // White glare highlight spot
      ctx.beginPath();
      ctx.ellipse(-3, -4, 5, 2, -0.2, 0, Math.PI * 2);
      ...
      // Pupils (Looking forward or dizzy X on game over)
      ...
      for (let i = s.obstacles.length - 1; i >= 0; i--) {
        ...
        if (s.gameState === 'playing') {
          ...
          if (px1 < ox2 && px2 > ox1 && py1 < oy2 && py2 > oy1) {
            ...
            // Explosion of burnt crumb particles
            for (let k = 0; k < 20; k++) {
              s.particles.push({
                x: p.x + 25,
                y: p.y + 20,
  ...
  return (
    <div className="min-h-screen bg-brand-charcoal text-white flex flex-col justify-between select-none relative overflow-hidden font-sans">
      ...
      {/* TOP NAVBAR / HEADER */}
      <header className="relative z-10 px-4 sm:px-8 py-5 flex items-center justify-between border-b border-white/10 backdrop-blur-md bg-brand-charcoal/80">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-brand-orange to-amber-600 flex items-center justify-center shadow-lg shadow-brand-orange/30">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 font-medium">Original Woodfire & Clay-Oven Dining</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          <button
            ...
            className="w-10 h-10 rounded-xl bg-white/10 hover:bg-white/20 border border-white/10 flex items-center justify-center text-white transition-all cursor-pointer"
          ...
          {onBackToFeast && (
            <button
              ...
              className="px-4 py-2 sm:px-5 sm:py-2.5 rounded-xl bg-gradient-to-r from-brand-orange to-amber-500 hover:from-brand-orange/90 hover:to-amber-500/90 text-white text-xs sm:text-sm font-black tracking-wide shadow-lg shadow-brand-orange/25 transition-all flex items-center gap-2 cursor-pointer active:scale-95"
            >
      ...
      <main className="relative z-10 flex-1 flex flex-col items-center justify-center px-4 py-6 max-w-4xl mx-auto w-full">
        ...
        <div className="text-center space-y-2 mb-4">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-brand-orange/15 border border-brand-orange/30 text-brand-orange text-xs font-black tracking-wider uppercase">
        ...
        <div className="relative w-full max-w-2xl bg-black/60 rounded-3xl p-3 sm:p-4 border border-brand-orange/30 shadow-2xl backdrop-blur-xl group">
          {/* Top Score HUD Bar */}
          <div className="flex items-center justify-between px-3 py-2 mb-2 rounded-2xl bg-white/5 border border-white/10 text-xs font-mono">
            ...
            <div className="hidden sm:inline-flex items-center gap-1 text-[11px] text-gray-400">
          ...
          <div
            ...
            className="relative w-full h-[220px] sm:h-[280px] rounded-2xl overflow-hidden cursor-pointer touch-none select-none bg-[#0a0d10] border border-white/10"
          >
            <canvas
              ref={canvasRef}
              width={750}
            ...
            <AnimatePresence>
              {milestoneText && (
                <motion.div
                  ...
                  className="absolute top-4 left-1/2 -translate-x-1/2 px-4 py-2 rounded-2xl bg-gradient-to-r from-brand-orange via-amber-500 to-brand-green text-white font-black text-xs sm:text-sm shadow-xl border border-white/20 flex items-center gap-2 pointer-events-none"
                >
            ...
            {gameState === 'idle' && (
              <div className="absolute inset-0 bg-black/60 backdrop-blur-xs flex flex-col items-center justify-center p-4 text-center">
                <div className="w-16 h-16 rounded-full bg-brand-orange/20 border border-brand-orange/40 flex items-center justify-center mb-3 animate-bounce">
                ...
                <div className="px-5 py-2.5 rounded-xl bg-brand-orange hover:bg-brand-orange/90 text-white font-black text-xs sm:text-sm shadow-lg tracking-wide animate-pulse">
                  TAP / SPACE TO JUMP
                </div>
              </div>
            )}
            ...
            {gameState === 'gameover' && (
              <div className="absolute inset-0 bg-black/75 backdrop-blur-md flex flex-col items-center justify-center p-4 text-center">
                <div className="text-xs font-black uppercase tracking-widest text-brand-orange mb-1">
                  KITCHEN CAUGHT!
                </div>
                <h3 className="text-2xl sm:text-3xl font-black text-red-400 mb-2">
                  GAME OVER
                </h3>
                <p className="text-xs text-gray-200 mb-4 max-w-sm font-medium">
                  {funnyQuote}
                </p>
                ...
                <div className="flex items-center gap-6 mb-5 text-sm font-mono">
                  ...
                  <div>
                    <span className="text-gray-400 text-xs block">BEST</span>
                ...
                <div className="flex items-center gap-3">
                  <button
                    ...
                    className="px-5 py-2.5 rounded-xl bg-brand-orange hover:bg-brand-orange/90 text-white font-black text-xs sm:text-sm shadow-lg flex items-center gap-2 cursor-pointer transition-all active:scale-95"
                  >
                  ...
                  {onBackToFeast && (
                    <button
                      ...
                      className="px-5 py-2.5 rounded-xl bg-white/10 hover:bg-white/20 text-white font-black text-xs sm:text-sm border border-white/20 transition-all cursor-pointer active:scale-95"
                    >
                      Return to Feast
                    </button>
                  )}
          ...
          {/* Controls Helper Tip */}
          <div className="mt-3 flex items-center justify-between text-[11px] text-gray-400 px-2 font-medium">
            <span>🕹️ Controls: <strong>[Space / Up Arrow]</strong> or <strong>Tap Screen</strong></span>
            <span>✨ <strong>Double-Tap</strong> in mid-air for a Butter Spin!</span>
          </div>
        </div>
        ...
        <div className="mt-6 flex flex-wrap items-center justify-center gap-3">
          <button
            onClick={onBackToFeast}
            className="px-5 py-2.5 rounded-xl bg-white/10 hover:bg-white/20 border border-white/15 text-white text-xs font-black transition-all cursor-pointer flex items-center gap-2"
          >
          ...
          <a
            ...
            className="px-5 py-2.5 rounded-xl bg-white/10 hover:bg-white/20 border border-white/15 text-white text-xs font-black transition-all cursor-pointer flex items-center gap-2"
          >
            <span>View Today's Bhatti Deals</span>
          </a>
          <a
            href="/#bhattis"
            className="px-5 py-2.5 rounded-xl bg-white/10 hover:bg-white/20 border border-white/15 text-white text-xs font-black transition-all cursor-pointer flex items-center gap-2"
          >
            <span>Dine-In Restaurant Locations</span>
          </a>
        </div>
      </main>

      {/* FOOTER */}
      <footer className="relative z-10 px-4 py-4 text-center text-xs text-gray-400 border-t border-white/10 bg-brand-charcoal/90">
